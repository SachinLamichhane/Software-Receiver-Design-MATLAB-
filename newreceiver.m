...
%% PLL preprocessing
f0 = 4*fc/fs;
fl = 200; ff = [0, f0-0.03, f0-0.02, f0+0.02, f0+0.03, 1]; fa = [0, 0, 1, 1, 0, 0];             % bandpass filter parameters
b = firpm(fl,ff,fa);
rp = filter(b,1,rsc);
...
mu1=.04; mu2=.012;                       % algorithm stepsizes
...
% b=[0.4 1 -0.6];              % define channel
% r = filter(b,1,r_tim);
%% dd equalizer
n=18; f=zeros(n,1);           % initialize equalizer
f(round(n/2)) = 1;
mu=.001;                       % stepsize
for i=n+1:m                  % iterate
...
% n=18; f=zeros(n,1);           % initialize equalizer at 0
% f(round(n/2)) = 1;
% mu=0.02; delta=9;             % stepsize and delay delta
